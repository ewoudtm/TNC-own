class SinglebidsController < ApplicationController
  ...
  def create
    ...
    if (bits.length >= 4)
      redirect_to productnegotiation_path(params[:productnegotiation_id]), alert: "Maximum amount of offers reached"
    elsif (params[:price].to_i < seventy_percent_startprice)
      redirect_to productnegotiation_path(params[:productnegotiation_id]), alert: "Minimum amount of the offer has to be at least 70% of the product price"
    elsif (params[:price].to_i < min_price)
      redirect_to productnegotiation_path(params[:productnegotiation_id]), alert: "Minimum offer must be at least 20 euros"
    else
